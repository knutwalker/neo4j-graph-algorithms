...
import org.neo4j.graphalgo.core2.write.PropertyTranslator;
 ...
 * long arraySize = 42L;
 * HugeHugeLongArray array = HugeHugeLongArray.newArray(arraySize, tracker);
 * array.set(13L, 37L);
 ...
 */
public abstract class HugeHugeLongArray {
     ...
     */
    abstract public void copyTo(final HugeHugeLongArray dest, long length);
     ...
     */
    public static HugeHugeLongArray newArray(long size, AllocationTracker tracker) {
        if (size <= SingleHugeHugeLongArray.PAGE_SIZE) {
            try {
                return SingleHugeHugeLongArray.of(size, tracker);
        ...
        return PagedHugeHugeLongArray.of(size, tracker);
    }

    /* test-only */
    static HugeHugeLongArray newPagedArray(long size, AllocationTracker tracker) {
        return PagedHugeHugeLongArray.of(size, tracker);
    }

    /* test-only */
    static HugeHugeLongArray newSingleArray(int size, AllocationTracker tracker) {
        return SingleHugeHugeLongArray.of(size, tracker);
    }
    ...
    /**
     * A {@link PropertyTranslator} for instances of {@link HugeHugeLongArray}s.
     */
    public static class Translator implements PropertyTranslator.OfLong<HugeHugeLongArray> {

        public static final Translator INSTANCE = new Translator();

        @Override
        public long toLong(final HugeHugeLongArray data, final long nodeId) {
            return data.get(nodeId);
        }
    }

    private static final class SingleHugeHugeLongArray extends HugeHugeLongArray {

        private static final int PAGE_SHIFT = 30;
        private static final int PAGE_SIZE = 1 << PAGE_SHIFT;

        private static HugeHugeLongArray of(long size, AllocationTracker tracker) {
            assert size <= PAGE_SIZE;
            final int intSize = (int) size;
            long[] page = new long[intSize];

            tracker.add(MemoryUsage.shallowSizeOfInstance(HugeHugeLongArray.class));
            tracker.add(MemoryUsage.sizeOfLongArray(intSize));

            return new SingleHugeHugeLongArray(intSize, page);
        }

        private final int size;
        private long[] page;

        private SingleHugeHugeLongArray(int size, long[] page) {
            this.size = size;
            this.page = page;
        }
        ...
        @Override
        public void copyTo(HugeHugeLongArray dest, long length) {
            if (length > size) {
                length = size;
            }
            if (length > dest.size()) {
            ...
            if (dest instanceof SingleHugeHugeLongArray) {
                SingleHugeHugeLongArray dst = (SingleHugeHugeLongArray) dest;
                System.arraycopy(page, 0, dst.page, 0, (int) length);
                Arrays.fill(dst.page, (int) length, dst.size, 0L);
            } else if (dest instanceof PagedHugeHugeLongArray) {
                PagedHugeHugeLongArray dst = (PagedHugeHugeLongArray) dest;
                int start = 0;
    ...
    public static final class PagedHugeHugeLongArray extends HugeHugeLongArray {
        ...
        private static HugeHugeLongArray of(long size, AllocationTracker tracker) {
            ...
            return new PagedHugeHugeLongArray(size, pages, memoryUsed);
        }
        ...
        private PagedHugeHugeLongArray(long size, long[][] pages, long memoryUsed) {
            this.size = size;
            this.pages = pages;
